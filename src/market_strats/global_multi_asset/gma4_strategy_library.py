...
This module contains target-weight rules only. It does not run replay accounting,
write files, select candidates, or interact with paper/broker workflows.
"""
...
DEFENSIVE_ASSETS = ["BIL", "IEF", "TLT", "AGG", "GLD", "DBC"]
...
def _dates_on_or_before(prices: dict[str, pd.DataFrame], symbol: str, date: Any) -> list[Any]:
    return [item for item in prices[symbol].index if item <= date]


def _price(
    prices: dict[str, pd.DataFrame], symbol: str, date: Any, field: str = "total_return_index"
) -> float:
    return float(prices[symbol].loc[date, field])


def _trailing_return(
    prices: dict[str, pd.DataFrame], symbol: str, date: Any, lookback_sessions: int
) -> float:
    dates = _dates_on_or_before(prices, symbol, date)
...
def _realised_volatility(
    prices: dict[str, pd.DataFrame], symbol: str, date: Any, lookback_sessions: int = 63
) -> float:
    dates = _dates_on_or_before(prices, symbol, date)
    if len(dates) <= 2:
        return 1.0
    use = dates[-min(len(dates), lookback_sessions + 1) :]
...
def _drawdown(
    ...
    dates = _dates_on_or_before(prices, symbol, date)
    use = dates[-min(len(dates), lookback_sessions) :]
    if not use:
        return 0.0
...
def _equal_weight(symbols: list[str]) -> dict[str, float]:
    if not symbols:
        return {BIL: 1.0}
    return _cap_with_bil_residual({symbol: 1.0 for symbol in symbols})
...
def _inverse_vol_weight(
    ...
    if not symbols:
        return {BIL: 1.0}
    raw = {
        symbol: 1.0 / _realised_volatility(prices, symbol, date, lookback_sessions)
...
def _single(symbol: str) -> Callable[[Any, dict[str, pd.DataFrame]], dict[str, float]]:
    def resolver(_date: Any, _prices: dict[str, pd.DataFrame]) -> dict[str, float]:
        return {symbol: 1.0}

    return resolver


def _equal_22(_date: Any, _prices: dict[str, pd.DataFrame]) -> dict[str, float]:
...
def _absolute_trend(
    ...
    def resolver(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
        bil_return = _trailing_return(prices, BIL, date, lookback)
        selected = [
            symbol
            for symbol in RISK_ASSETS
            if _trailing_return(prices, symbol, date, lookback) > bil_return
        ]
        if weighting == "inverse_volatility":
            return _inverse_vol_weight(prices, selected, date)
...
def _xsmom(
    ...
    def resolver(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
        bil_return = _trailing_return(prices, BIL, date, lookback)
        ranked = sorted(
            [(symbol, _trailing_return(prices, symbol, date, lookback)) for symbol in RISK_ASSETS],
            key=lambda item: item[1],
            reverse=True,
        )
        selected = [symbol for symbol, value in ranked if value > bil_return][:top_n]
        if weighting == "inverse_volatility":
            return _inverse_vol_weight(prices, selected, date)
        return _equal_weight(selected)
...
def _mean_reversion(
    lookback_days: int,
    long_trend_filter: bool = False,
) -> Callable[[Any, dict[str, pd.DataFrame]], dict[str, float]]:
    def resolver(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
        ranked = sorted(
            [
                (symbol, _trailing_return(prices, symbol, date, lookback_days))
                for symbol in RISK_ASSETS
            ],
            key=lambda item: item[1],
        )
        selected = [symbol for symbol, _value in ranked[:3]]
        if long_trend_filter:
            selected = [
                symbol for symbol in selected if _above_moving_average(prices, symbol, date, 200)
            ]
        return _equal_weight(selected)
...
def _defensive_inverse_vol(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
    return _inverse_vol_weight(prices, FIXED_GMA4_UNIVERSE, date)
...
def _spy_200d_rotation(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
    if _above_moving_average(prices, "SPY", date, 200):
        return _risk_on_diversified()
    return _risk_off_defensive()


def _drawdown_guard(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
    if _drawdown(prices, "SPY", date, 252) <= -0.10:
        return _risk_off_defensive()
    return _risk_on_diversified()
...
def _blend(
    ...
    def resolver(date: Any, prices: dict[str, pd.DataFrame]) -> dict[str, float]:
        ...
        for component_id, component_weight in zip(component_ids, weights):
            component = build_gma4_trial_rules()[component_id].resolver(date, prices)
...
def build_gma4_trial_rules() -> dict[str, GMA4TrialRule]:
    return {
        "gma4_benchmark_bil_buy_hold_v1": GMA4TrialRule(
            "gma4_benchmark_bil_buy_hold_v1", "monthly_last_session_next_open", _single("BIL")
        ),
        "gma4_benchmark_spy_buy_hold_v1": GMA4TrialRule(
            "gma4_benchmark_spy_buy_hold_v1", "monthly_last_session_next_open", _single("SPY")
        ),
        "gma4_benchmark_equal_weight_22_monthly_v1": GMA4TrialRule(
            "gma4_benchmark_equal_weight_22_monthly_v1", "monthly_last_session_next_open", _equal_22
        ),
        ...
        "gma4_meanrev_5d_bottom3_equal_weight_v1": GMA4TrialRule(
            "gma4_meanrev_5d_bottom3_equal_weight_v1", "weekly_friday_next_open", _mean_reversion(5)
        ),
        ...
        "gma4_defensive_inverse_vol_63d_v1": GMA4TrialRule(
            "gma4_defensive_inverse_vol_63d_v1", "weekly_friday_next_open", _defensive_inverse_vol
        ),
        "gma4_defensive_spy_200d_rotation_v1": GMA4TrialRule(
            "gma4_defensive_spy_200d_rotation_v1", "weekly_friday_next_open", _spy_200d_rotation
        ),
        "gma4_defensive_drawdown_guard_v1": GMA4TrialRule(
            "gma4_defensive_drawdown_guard_v1", "weekly_friday_next_open", _drawdown_guard
        ),
