...
def run_buy_and_hold(prices: pd.DataFrame, initial_capital: float) -> pd.DataFrame:
    """
    Buy SPY at the start and hold until the end.

    Uses adjusted close returns.
    """
    df = prices.copy()
    ...
    return df[["date", "adj_close", "strategy_return", "equity", "position", "turnover"]]