...
def _load_price_frames(
    *, combined_dir: Path, post_dir: Path, pilot_dir: Path
...
def build_mark_to_market_histories(
    *,
    ...
    execution_date = _date_string(
        session.get("observed_execution_date") or session.get("expected_execution_date")
    )
    ...
    missing_frames = [
        ticker for ticker in required if ticker not in price_frames or price_frames[ticker].empty
    ]
    if missing_frames:
        return (
            pd.DataFrame(columns=POSITION_VALUATION_COLUMNS),
    ...
    for ticker in required:
        candidate_dates |= {
            date for date in _available_dates(price_frames[ticker]) if date > execution_date
        }
    complete_dates: list[str] = []
    blockers: list[str] = []
    for date in sorted(candidate_dates):
        missing = [
            ticker for ticker in required if not _completed_close_on(price_frames[ticker], date)[0]
        ]
        if missing:
    ...
    if existing_position_history is not None and not existing_position_history.empty:
        position_history = pd.concat(
            [existing_position_history, position_history], ignore_index=True
        )
    ...
    if existing_portfolio_history is not None and not existing_portfolio_history.empty:
        portfolio_history = pd.concat(
            [existing_portfolio_history, portfolio_history], ignore_index=True
        )
...
def build_order_packet(
    *,
    ...
    for _, row in current_target.iterrows():
        ticker = str(row["ticker"])
        target_shares = _safe_int(
            row.get("execution_target_shares", row.get("estimated_target_shares", 0))
        )
    ...
    targets = current_target.copy()
    targets["target_shares"] = targets.get(
        "execution_target_shares", targets.get("estimated_target_shares", 0)
    )
    ...
    *,
    ...
    if missing:
        return (
            ...
            dict(starting_shares),
            cash_balance,
            [f"missing_fill_columns:{';'.join(missing)}"],
        )
    ...
    packet_keys = (
        set(map(tuple, packet[key_cols].astype(str).to_numpy())) if not packet.empty else set()
    )
    ledger_rows = []
        ...
        ledger_rows.append(
            {
                ...
                "ledger_status": "accepted_manual_tradingview_fill"
                if status in {"filled", "partially_filled"}
                else status,
            }
        )
...
def _target_holdings_from_targets(
    targets: pd.DataFrame, actual_shares: dict[str, int]
) -> pd.DataFrame:
    if targets.empty:
        rows = [
            {
        ...
        rows.append(
            {
                "ticker": str(row["ticker"]),
                "target_shares": _safe_int(
                    row.get("target_shares", row.get("execution_target_shares", 0))
                ),
                "target_weight": _safe_float(row.get("target_weight")),
...
def _tracking_reconciliation(
    *,
    ...
    tickers = sorted(
        set(targets["ticker"].astype(str)) | set(actual_shares) | set(packet["ticker"].astype(str))
    )
    rows = []
    packet_delta = (
        {
            str(row["ticker"]): (
                _safe_int(row["order_quantity"])
                if row["side"] == "BUY"
                else -_safe_int(row["order_quantity"])
            )
            for _, row in packet.iterrows()
        }
        if not packet.empty
    ...
    target_map = (
        {str(row["ticker"]): _safe_int(row["target_shares"]) for _, row in targets.iterrows()}
        ...
        rows.append(
            {
                ...
                "tracking_difference_notional": diff * latest_price
                if np.isfinite(latest_price)
                else np.nan,
...
def save_phase23l_operational_paper_bridge(
    *,
    ...
    elif valuation_blockers and any(
        "missing_completed_close" in item for item in valuation_blockers
    ):
    ...
    _write_csv(
        position_history,
        output_dir / "phase23l_position_valuation_history.csv",
        POSITION_VALUATION_COLUMNS,
    )
    _write_csv(
        portfolio_history,
        output_dir / "phase23l_portfolio_valuation_history.csv",
        PORTFOLIO_VALUATION_COLUMNS,
    )
    ...
    _write_csv(
        fill_template,
        output_dir / "phase23l_tradingview_manual_fill_template.csv",
        FILL_TEMPLATE_COLUMNS,
    )
