...
DEFAULT_PHASE11E_CONFIG: dict[str, Any] = {
    "enabled": False,
    "implementation_role": ("Regime scoring diagnostic panel template implementation audit only"),
    ...
    "template_policy": {
        "component_availability_status_source": ("Phase 11D component availability spec"),
...
def _required_columns_by_report(required_columns_spec: pd.DataFrame) -> dict[str, list[str]]:
    ...
    for report_name, group in required_columns_spec.groupby("report_name"):
        grouped[str(report_name)] = [
            str(item) for item in group["required_column"].dropna().astype(str).tolist()
        ]
        ...
        rows.append(
            {
                ...
                "unavailable_reason": "",
                "blocked_reason": "blocked_pending_source_leakage_audit" if is_blocked else "",
                "future_unblock_requirement": str(item.get("future_unblock_requirement", "")),
            }
        )
        ...
        rows.append(
            {
                ...
                "missingness_reason": "blocked_pending_source_leakage_audit" if is_blocked else "",
        ...
        rows.append(
            {
                ...
                "numeric_weight_allowed": _bool_value(item.get("numeric_weight_allowed", False)),
        ...
        rows.append(
            {
                ...
                "current_use_allowed": _bool_value(item.get("current_use_allowed", False)),
                "score_component_allowed": _bool_value(item.get("score_component_allowed", False)),
            }
        )
...
def build_phase11e_schema_compliance_report(
    *,
    ...
    frame = pd.DataFrame(rows)
    frame["result"] = frame["schema_passed"].map({True: "Passed", False: "Failed"})
    ...
    rows = [
        {
            ...
            "passed": "content audit" in str(boundary.get("allowed_next_step", "")).lower(),
        },
        {
...
def build_phase11e_summary(
    *,
    ...
    schema_compliance_passed = (
        bool(schema_compliance["schema_passed"].all()) if not schema_compliance.empty else False
    )
    ...
    return pd.DataFrame(
        [
            {
                "implementation_role": str(phase_config.get("implementation_role", "")),
                ...
                "phase11f_boundary_passed": bool(phase11f_boundary_check["passed"].all())
    ...
    *,
    ...
    rows = [
        _gate_row(
            ...
            (f"source_design_reports_present={bool(row['source_design_reports_present'])}"),
        ),
        _gate_row(
            "Template reports are generated",
            (not gates.get("require_template_reports_generated", True))
            or int(row["template_report_count"]) >= int(gates.get("min_template_reports", 6)),
        ...
        _gate_row(
            ...
            or int(row["missingness_rows"]) >= int(gates.get("min_missingness_rows", 5)),
        ...
        _gate_row(
            ...
            or int(row["weighting_policy_rows"]) >= int(gates.get("min_weighting_policy_rows", 5)),
        ...
        _gate_row(
            ...
            or int(row["blocked_family_rows"]) >= int(gates.get("min_blocked_family_rows", 2)),
        ...
        _gate_row(
            ...
            (f"missingness_no_return_inference={bool(row['missingness_no_return_inference'])}"),
        ),
        _gate_row(
...
def save_phase11e_regime_scoring_diagnostic_panel_template_audit(
    *,
    ...
    source_tables = {
        "panel_layout_spec": _read_csv_if_exists(source_report_paths.get("panel_layout_spec", "")),
    ...
    write_phase11e_markdown(
        ...
        output_path=reports_path / "phase11e_regime_scoring_diagnostic_panel_template_audit.md",
    )
