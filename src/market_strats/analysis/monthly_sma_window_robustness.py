...
def _classify_monthly_sma_window(
...
def run_monthly_sma_window_robustness(
    ticker: str,
    prices: pd.DataFrame,
    ...
    """
    Test whether a monthly SMA result is robust across neighbouring windows.

    This is designed for the EFA 10-month SMA lead. It is not an optimisation
    sweep. It checks whether the result sits inside a stable monthly trend
    region.
    """
    if not sma_months:
        raise ValueError("sma_months cannot be empty")
    ...
    for sma_month in sma_months:
        strategy_name = f"{sma_month}-Month SMA"

        result = run_sma_trend_strategy(
            prices=prices,
            initial_capital=initial_capital,
            sma_months=sma_month,
        ...
        rows.append(
            {
                ...
                "classification": _classify_monthly_sma_window(
    ...
    return output.sort_values("sma_months").reset_index(drop=True)


def create_monthly_sma_window_robustness_summary(
    robustness: pd.DataFrame,
    anchor_sma_months: int = 10,
) -> pd.DataFrame:
    if robustness.empty:
        return pd.DataFrame()

    if anchor_sma_months not in set(robustness["sma_months"]):
        raise ValueError(f"anchor_sma_months={anchor_sma_months} not found in results")

    anchor = robustness[robustness["sma_months"] == anchor_sma_months].iloc[0]

    neighbouring = robustness[
        robustness["sma_months"].isin(
            [anchor_sma_months - 2, anchor_sma_months, anchor_sma_months + 2]
        )
    ].copy()
    ...
    return pd.DataFrame(
        [
            {
                "anchor_sma_months": anchor_sma_months,
...
def write_monthly_sma_window_robustness_markdown(
    robustness: pd.DataFrame,
...
This report tests whether a monthly SMA result is robust across neighbouring windows.

It is designed to validate whether an observed monthly trend signal is part of a stable region or a one-window spike.
...
- A robust result should not depend on one exact monthly SMA window.
